...
public class Player : MonoBehaviour
{
    ...
    public int experience = 100; // int that we are seting for some goal
    public int level = 1; // int that says witch level we are on
    public TextMeshProUGUI scoreText; // text that is going to display our score
    ...
    private void Update()
    {
        ...
        scoreText.text = "Experience = " + experience; // seting what will the text say
        
        if(Input.GetMouseButtonDown(0) && mainMenu.activeSelf == false && endScreen.activeSelf == false) // if that is looking if we are pressing the screen if mainMenu and endScreen are turned off
        {
    ...
    private void OnTriggerEnter(Collider other) // method that dies something when we enter a colider
    {
        ...
        if(other.gameObject.tag == "coin") // if you go into a colider with a tag coin
        {
            ...
            other.gameObject.SetActive(false); // deactivating coin object
        }
    }
}
